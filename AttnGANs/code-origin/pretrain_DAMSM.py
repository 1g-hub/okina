...
def parse_args():
    ...
    parser.add_argument('--cfg', dest='cfg_file', help='optional config file', default='cfg/DAMSM/original.yml', type=str)
...
def train(dataloader, cnn_model, rnn_model, batch_size, labels, optimizer, epoch, ixtoword, image_dir):
    ...
    for step, data in enumerate(dataloader, 1):
        ...
        # words_features: batch_size x embedding_dim x 17 x 17
        # sent_code: batch_size x embedding_dim
        words_features, sent_code = cnn_model(imgs[-1])
        # --> batch_size x embedding_dim x 17*17
        embedding_dim, att_sze = words_features.size(1), words_features.size(2)
        # words_features = words_features.view(batch_size, embedding_dim, -1)

        hidden = rnn_model.init_hidden(batch_size)
        # words_emb: batch_size x embedding_dim x seq_len
        # sent_emb: batch_size x embedding_dim
        words_emb, sent_emb = rnn_model(captions, cap_lens, hidden)

        w_loss_1, w_loss_2, attn = words_loss(words_features, words_emb, labels, cap_lens, class_ids, batch_size, epoch, mode='train')
        w_total_loss_1 += w_loss_1.data
        w_total_loss_2 += w_loss_2.data
        w_loss = w_loss_1 + w_loss_2

        s_loss_1, s_loss_2 = sent_loss(sent_code, sent_emb, labels, class_ids, batch_size)
    ...
    # attention Maps
    img_set, _ = build_super_images(imgs[-1].cpu(), captions, ixtoword, attn, att_sze)
...
def evaluate(dataloader, cnn_model, rnn_model, batch_size, epoch, ixtoword, image_dir):
    ...
    for step, data in enumerate(dataloader, 1):
        ...
        words_features, sent_code = cnn_model(imgs[-1])
        # embedding_dim = words_features.size(1)
        embedding_dim, att_sze = words_features.size(1), words_features.size(2)
        # words_features = words_features.view(batch_size, embedding_dim, -1)
        ...
        w_loss_1, w_loss_2, attn = words_loss(words_features, words_emb, labels, cap_lens, class_ids, batch_size, epoch, mode='test')
        w_total_loss_1 += w_loss_1.data
        w_total_loss_2 += w_loss_2.data
    ...
    eval = [w_cur_loss_1, w_cur_loss_2, s_cur_loss_1, s_cur_loss_2]
